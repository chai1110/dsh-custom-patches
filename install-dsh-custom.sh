...
#
# 本 tag 固定适配 DSH 0.1.0-rc.7（其他版本见 README「多版本支持」的 tag 列表）
# =============================================================================
set -u
...
# 本仓库（tag v0.1.0-rc.7）固定适配的 DSH 版本：0.1.0-rc.7。
# 老版本 / 其他版本用户：请 clone 后 checkout 对应版本 tag（见 README「多版本支持」）。
TARGET_VERSION="0.1.0-rc.7"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *)
      ...
      echo "  （本 tag 固定适配 DSH 0.1.0-rc.7；其他版本请 checkout 对应 tag）"
      exit 1
      ;;
  esac
done
